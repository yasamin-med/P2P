#!/bin/bash
#SBATCH --job-name=s5_others # Specify the job name
#SBATCH --account=st-ilker-1-gpu   # Specify your allocation code
#SBATCH --nodes=1              # Request 1 node
#SBATCH --ntasks=1             # Request 1 task
#SBATCH --gpus-per-node=4
#SBATCH --mem=32G               # Request 32 GB of memory
#SBATCH --time=4:00:00        # Request 10 hours of runtime
...
for split in "${splits[@]}"; do
  for class in "${classes[@]}"; do
    for classifier in "${classifiers[@]}"; do
      ...
      #export DATA_DIR="/arc/project/st-ilker-1/yasamin/Breast_missing_images/${classifier}/split${split}/${class}"
      export OUTPUT_DIR="textual_inversion_${class}_split_new${split}"
      export SAVING_DIR="/scratch/st-ilker-1/yasamin/Textual_attack/images_splits_cross_others/${classifier}/split${split}/${class}"
      export Classifier_path="/scratch/st-ilker-1/yasamin/Breast/result_split_${split}/result_original"
      export csv_path="/scratch/st-ilker-1/yasamin/Textual_attack/images_splits_cross_others/${classifier}/split${split}/${class}/attack_info.csv"
      ...
      accelerate launch /scratch/st-ilker-1/yasamin/Textual_attack/textual_inversion_new_breast_loop_cross_others.py \
        --pretrained_model_name_or_path=$MODEL_NAME \
        --train_data_dir=$DATA_DIR \
        --learnable_property="object" \
