...
# If it's the first iteration, create the file and write the header
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        attack_flag = 0
        for epoch in range(first_epoch, 200): #args.num_train_epochs
            with accelerator.accumulate(text_encoder):
               ...
               # Convert images to latent space
                ...
                #mine
                # before 100
                timesteps = torch.randint(0, 50, (bsz,), device=latents.device)
                ...
                # image_orig_pil = img_orig.squeeze(0).detach().cpu().numpy().transpose(1, 2, 0)
                # image_orig_pil = (image_orig_pil * 255).astype(np.uint8)
                # image_orig_pil = Image.fromarray(image_orig_pil)
                # image_orig_pil.save(args.saving_image_dir + "/img_orig.png")
                ...
                # Remove the item equal to label
                if label in my_list:
                    my_list.remove(label)

                # Unpack the remaining items
                remaining_item1, remaining_item2 = my_list

                # Compute the losses using the remaining items
                loss_cross_entropy_1 = criterion(predicted_class, torch.tensor([remaining_item1], device=accelerator.device))
                loss_cross_entropy_2 = criterion(predicted_class, torch.tensor([remaining_item2], device=accelerator.device))

                # loss_cross_entropy_mal = criterion(predicted_class ,torch.tensor([1], device=accelerator.device))
                # loss_cross_entropy_normal = criterion(predicted_class ,torch.tensor([2], device=accelerator.device))
                #negative_loss_cross_entropy_benign = -1 * criterion(predicted_class ,label)
                loss_cross_entropy =  loss_cross_entropy_1 + loss_cross_entropy_2
                #loss_cross_entropy = negative_loss_cross_entropy_benign
                #loss_mse =  F.mse_loss(batch["pixel_values"].to(dtype=weight_dtype), image_adver, reduction="mean")

                #loss_mse =  F.mse_loss(batch["pixel_values"].float(), image_adver[0].unsqueeze(0).float(), reduction="mean")
                loss_mse =  F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                #loss_max_margin = ( predicted_class_softmax[0][label] - predicted_class_softmax[0][class_second_max])
                # relaxation_loss = 1e11 * F.mse_loss(predicted_class_softmax.float(), predicted_class_orig_softmax.float(), reduction="mean")
                # loss =   loss_max_margin - (landa - (epoch + 1) * 0.01) * relaxation_loss
                loss =  loss_cross_entropy / 20 +   loss_mse
                #print("loss nll:", loss_nll.cpu())
                
                
                print("loss mse:", loss_mse.cpu())
                # print("loss maximum margin :",loss_max_margin.cpu())
                # print("relaxation loss: " , relaxation_loss.cpu())
                print("loss related to class malignant: " , loss_cross_entropy_1.cpu() / 20)
                print("loss related to class normal: " , loss_cross_entropy_2.cpu() / 20)
                ...
                # embedding_grad = text_encoder.get_input_embeddings().weight.grad
                # if embedding_grad is None:
                #     print("No gradients for the embeddings!")
                # else:
                #     print("Gradients for embeddings exist.")
                #     print("Gradient norms:", torch.norm(embedding_grad, dim=-1))
                #print("Before step, embedding of token:", text_encoder.get_input_embeddings().weight[placeholder_token_ids].clone())
                optimizer.step()
                #print("After step, embedding of token:", text_encoder.get_input_embeddings().weight[placeholder_token_ids].clone())
                lr_scheduler.step()
                optimizer.zero_grad()
                ...
                #print("original prompt" , tokenizer.convert_ids_to_tokens(batch["input_ids"]))
                    ...
                    if torch.all(class_max_orig == label):
                        if timesteps.cpu() < 50:
                            ...
                            if name[0] in image_dict:
                                ...
                                mse_best = F.mse_loss(batch["pixel_values"].float(), image_adver[0].unsqueeze(0).float(), reduction="mean")
                                class_attack_best = class_max.cpu()
                            #cv2.imwrite(f"/scratch/st-ilker-1/yasamin/Textual_attack/corrupted_examples_dreambooth_coeff_20/img_{name}_{timesteps.cpu()}_{count}.jpg", img_save)
                            #image_adver[0].save(args.saving_image_dir + f"/img_{name}_{timesteps.cpu()}_{count}.png")
