...
parser = argparse.ArgumentParser(description='LPIPS Calculation',
...
def load_classifier(name, classifier_path):
    if name == 'squeezenet1.1':
        net = models.squeezenet1_1(weights=False)
        ...
        net = models.densenet121(weights=False)
...
for classifier in ["resnet34" , "squeezenet1.1" , "densenet121"]:
    ...
    for class_name in ["benign", "malignant"]:
        ...
        for split in ["1", "2", "3", "4", "5"]:
            ...
            total_lpips = 0.0
            print(orig_path)
            print(list_image_orig)
            for name in list_image_orig:
                image_orig_path = os.path.join(orig_path, name)
                #image_attack_path = os.path.join(attack_path, f"img_{name}_best.png")
                ...
                if class_orig == label:
                    ...
                    lpips_value = loss_fn(image_orig, image_attack)
                    print(lpips_value)
                    total_lpips += lpips_value.item()
            
            split_row.append(total_lpips / len(list_image_orig) if len(list_image_orig) > 0 else 0.0)
        
        class_row.append(np.mean(split_row) if split_row else 0.0)
    
    avg_lpips = np.mean(class_row) if class_row else 0.0
    row = [classifier, class_row[0], class_row[1], avg_lpips]
    with open(csv_file_path, 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(row)
        
    print(f"avg LPIPS for {classifier}: {avg_lpips}")
